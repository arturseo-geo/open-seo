...
export const savedKeywords = sqliteTable(
  "saved_keywords",
  {
    ...
    aiCluster: text("ai_cluster"), // Populated dynamically via Cloudflare Llama 3
    createdAt: text("created_at")
      .notNull()
      .default(sql`(current_timestamp)`),
  },
...
// ============================================================================
// LLM Verification & AI Tracking tables
// ============================================================================

// Query Sets for AI presence tracking
export const llmQuerySets = sqliteTable("llm_query_sets", {
  id: text("id").primaryKey(),
  projectId: text("project_id")
    .notNull()
    .references(() => projects.id, { onDelete: "cascade" }),
...
// Queries belonging to a Query Set
export const llmTrackedQueries = sqliteTable("llm_tracked_queries", {
  id: text("id").primaryKey(),
  querySetId: text("query_set_id")
    .notNull()
    .references(() => llmQuerySets.id, { onDelete: "cascade" }),
...
// Logs of AI citations over time
export const llmCitations = sqliteTable("llm_citations", {
  id: text("id").primaryKey(),
  queryId: text("query_id")
    .notNull()
    .references(() => llmTrackedQueries.id, { onDelete: "cascade" }),
  engine: text("engine", { enum: ["chatgpt", "perplexity", "gemini"] }).notNull(),
  attributionType: text("attribution_type", { enum: ["domain", "url", "brand"] }).notNull(),
  targetMatch: text("target_match").notNull(),
  llmResponseText: text("llm_response_text"), // Full snippet or answer contextualizing the mention
  sentiment: text("sentiment").notNull().default("neutral"), // Positive, Negative, or Neutral
  mentionedAt: text("mentioned_at")
    .notNull()
    .default(sql`(current_timestamp)`),
});
...
export const rankings = sqliteTable(
  ...
  (table) => [
    index("rankings_project_keyword_idx").on(
      table.projectId,
      table.keyword,
      table.date,
    ),
