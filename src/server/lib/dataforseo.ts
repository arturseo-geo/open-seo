...
// ---------------------------------------------------------------------------
// LLM Verification & Mentions wrapper
// ---------------------------------------------------------------------------

export async function fetchLlmMentionsRaw(
  keyword: string,
  engine: "chatgpt" | "perplexity" | "gemini"
): Promise<any[]> {
  // DataForSEO AI Optimization endpoint uses exact underscores for chat_gpt
  const engineSlug = engine === "chatgpt" ? "chat_gpt" : engine;
  ...
  const endpoint = `/v3/ai_optimization/${engineSlug}/llm_responses/live`;
  const responseRaw = await postDataforseo(endpoint, [
    {
      user_prompt: keyword,
  ...
  // This structure models standard DataForSEO response arrays
