...
export const getRankHistory = createServerFn({ method: "POST" })
  .middleware(authenticatedServerFunctionMiddleware)
  ...
  .handler(async ({ data }) => KeywordResearchService.getRankHistory(data));

export const getLatestProjectRankings = createServerFn({ method: "POST" })
  .middleware(authenticatedServerFunctionMiddleware)
  .inputValidator((data: unknown) =>
    z.object({ projectId: z.string().min(1) }).parse(data),
  )
  .handler(async ({ data }) =>
    KeywordResearchService.getLatestProjectRankings(data.projectId),
  );
